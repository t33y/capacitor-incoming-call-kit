...
export interface PushNotificationsPlugin {
    ...
    addListener(eventName: 'onIceCandidate', cb: (data: {
        iceCandidate: RTCIceCandidate;
    }) => void): Promise<PluginListenerHandle>;
    addListener(eventName: 'onConnectionStateChange', cb: (data: {
        connectionState: RTCIceConnectionState;
    }) => void): Promise<PluginListenerHandle>;
    addListener(eventName: 'onDataReceived', cb: (data: {
        dataReceived: string;
    }) => void): Promise<PluginListenerHandle>;
    addListener(eventName: 'videoIntentConfirm', cb: (data: {
        confirm: string;
    }) => void): Promise<PluginListenerHandle>;
    addListener(eventName: 'videoIntenthandle', cb: (data: {
        contact: string;
...
export declare type Responses = void | {
    isMuted: boolean;
} | {
    calls: CallKitParams[];
} | {
...
export interface FlutterCallkitIncomingPlugin extends PushNotificationsPlugin {
    unregisterProximityListener(): Promise<{
        status: string;
    }>;
    ...
    setRemoteSdp({ sdp, type, iceServers, }: {
        sdp: string;
        type: string;
        iceServers?: [
            {
    ...
    setRemoteIceCandidate({ sdpMLineIndex, sdpMid, candidate }: RTCIceCandidateInit): Promise<{
        status: string;
    }>;
    closePeerConnection(): Promise<{
        status: string;
    }>;
    getPeerConnectionStatus(): Promise<{
        status: string;
    }>;
    getRemoteDescriptionStatus(): Promise<{
        status: string;
    }>;
    doMethod(options: {
        options: string;
